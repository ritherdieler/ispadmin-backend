...
class DeployStagingIsolationScriptsTest {
    ...
    fun composeHelperAddsTomcatStagingWithoutTouchingProd(@TempDir tmp: Path) {
        ...
        val stagingBlock = text.substringAfter("  tomcat-staging:")
        assertTrue(stagingBlock.contains("OLT_GATEWAY_WRITES_ENABLED"), stagingBlock)
        assertTrue(stagingBlock.contains("OLT_GATEWAY_ENABLED: \"false\""), stagingBlock)
        assertFalse(prodBlock.contains("OLT_GATEWAY_WRITES_ENABLED"), prodBlock)
        assertFalse(prodBlock.contains("OLT_GATEWAY_ENABLED"), prodBlock)
