...
class OltServiceSecretsPropertiesFileTest {
    ...
    fun no_queda_ninguna_credencial_de_smartolt_compilada_en_el_codigo() {
        val configConstants = root.resolve(
            "core/src/main/kotlin/com/dscorp/wispadmin/wispadmin/config/ConfigConstants.kt"
        )
        ...
        val kotlinRoots = listOf("core", "acs", "oltgateway", "traffic", "shared", "events", "transport", "routeros", "servicehealth", "netdiag", "observability").map {
            root.resolve("$it/src/main/kotlin")
        }
        val offenders = kotlinRoots.filter { Files.isDirectory(it) }.flatMap { kotlinRoot ->
