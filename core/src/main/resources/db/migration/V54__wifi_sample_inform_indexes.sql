...
CREATE INDEX `idx_sh_wifi_device_inform`
  ON `acs_wifi_count_sample` (`device_id`, `subscription_id`, `inform_at`);

CREATE INDEX `idx_sh_wifi_inform_at`
  ON `acs_wifi_count_sample` (`inform_at`);

CREATE INDEX `idx_sh_station_observed_at`
  ON `acs_wifi_station_sample` (`observed_at`);
