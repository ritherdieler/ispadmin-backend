...
-- staging Hibernate: lab es NOT NULL sin default; onu puede no existir (inventario en olt_mgr_onu).

INSERT INTO subscription_acs (subscription_id, genieacs_device_id, last_error, provision_status, ssid_24, ssid_5, updated_at, lab)
SELECT s.id, s.tr069_device_id, LEFT(s.tr069_last_error, 500), s.tr069_provision_status, s.wifi_ssid_24, s.wifi_ssid_5, NOW(), 0
FROM subscription s
...
BEGIN
    ...
    ELSE
        ...
        IF EXISTS (
            ...
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'onu'
        ) THEN
            SET @cnt = CONCAT(
