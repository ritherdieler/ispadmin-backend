...
class IdentityService(
    ...
    fun resolveAcs(deviceId: String): Int? {
        val ids = directory.findIdsByTr069DeviceId(deviceId).distinct()
        if (ids.size != 1) return null
        val id = ids.single()
        val otherOperational = directory.find(id)?.tr069DeviceId
    ...
    fun resolveOnu(sn: String): Int? {
        val exact = directory.findIdsByOnuSerial(sn)
        if (exact.size == 1) return exact.single()
        if (exact.size > 1) return null
        val suffix = OnuSerialSuffix.normalizeSuffix(sn) ?: return null
        return directory.findIdsByOnuSerialOrSuffix(sn, suffix).singleOrNull()
    }
    ...
    fun resolveOnuForCollection(sn: String): Int? {
        ...
        if (exact.size == 1) return exact.single()
        val suffix = OnuSerialSuffix.normalizeSuffix(sn) ?: return null
        val candidates = directory.findIdsByOnuSerialOrSuffix(sn, suffix)
    ...
    fun reconcile(id: Int, now: Instant = Instant.now()): Map<String, String> {
        val verified = directory.lockIdentityOwner(id) ?: return emptyMap()
