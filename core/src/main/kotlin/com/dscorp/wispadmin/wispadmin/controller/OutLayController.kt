...
class OutLayController(
    ...
    fun createOutlay(
        ...
        if (image != null && image.isNotEmpty()) {
            receiptUrls = image.mapNotNull { receipt ->
                if (!receipt.isEmpty) {
                    try {
                        storageService.uploadFileToFolder(receipt, "outlays")
                    } catch (e: Exception) {
                        null
                    }
                } else {
        ...
        val outlay = Outlay(
            ...
            receipt_url = receiptUrls.joinToString(","),
            responsible = user,
            date = java.util.Date()
        )

        val savedOutlay = outlayRepository.save(outlay)
        return ResponseEntity.status(201).body(savedOutlay.toDto())
    }
    ...
    fun updateOutlay(@PathVariable id: Int, @RequestBody outlayDTO: OutlayDTO, @RequestParam userId: Int): ResponseEntity<OutlayDTO> {
        ...
        return ResponseEntity.ok(savedOutlay.toDto())
    }
