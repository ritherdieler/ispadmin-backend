package com.dscorp.wispadmin.shared.security
...
class ObservabilitySessionTokenService(
    private val properties: SessionTokenProperties,
    ...
    fun isConfigured(): Boolean = properties.secret.isNotBlank()

    fun issueAccess(userId: Int?, username: String?, type: String?): String? =
        issue(userId, username, type, TYPE_ACCESS, properties.ttlMinutes)

    fun issueRefresh(userId: Int?, username: String?, type: String?): String? =
        issue(userId, username, type, TYPE_REFRESH, properties.refreshTtlMinutes)
