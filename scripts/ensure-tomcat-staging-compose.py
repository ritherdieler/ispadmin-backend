...
def ensure_olt_writes(block):
    ...
    for line in block:
        if line.strip() == "environment:" or line.lstrip().startswith("environment:"):
            has_env = True
        if WRITES_KEY in line:
            has_writes = True
            indent = line[: len(line) - len(line.lstrip())]
            nl = "\n" if line.endswith("\n") else ""
            if line.lstrip().startswith("-"):
                new = f"{indent}- {WRITES_KEY}={WRITES_VALUE}{nl}"
            else:
                new = f"{indent}{WRITES_KEY}: {WRITES_VALUE}{nl}"
            if line != new:
                changed = True
    ...
    if has_env:
        for line in out:
            ...
            if not injected and PROFILE_KEY in line:
                indent = line[: len(line) - len(line.lstrip())]
                if line.lstrip().startswith("-"):
                    inserted.append(f"{indent}- {WRITES_KEY}={WRITES_VALUE}{nl}")
                else:
                    inserted.append(f"{indent}{WRITES_KEY}: {WRITES_VALUE}{nl}")
                injected = True
        if injected:
        ...
        for i, line in enumerate(inserted):
            if line.strip() == "environment:" or line.lstrip().startswith("environment:"):
                indent = line[: len(line) - len(line.lstrip())]
                inserted.insert(i + 1, f"{indent}  {WRITES_KEY}: {WRITES_VALUE}{nl}")
                return inserted, True
        inserted.append(f"      {WRITES_KEY}: {WRITES_VALUE}{nl}")
        return inserted, True
    inserted.extend(out)
    inserted.append(f"    environment:{nl}")
    inserted.append(f"      {WRITES_KEY}: {WRITES_VALUE}{nl}")
