...
function customerDualIpWan() {
  const values = {};
  values["InternetGatewayDevice.ManagementServer.ConnectionRequestURL"] = "http://192.168.255.169:7547/tr069";
  values[WCD1 + ".WANIPConnection.1.ExternalIPAddress"] = "192.168.211.145";
  values[WCD1 + ".WANIPConnection.1.X_CT-COM_VLANIDMark"] = 1;
  values[WCD1 + ".X_CT-COM_WANGponLinkConfig.VLANIDMark"] = 1;
  values[VSOL_TR069 + ".ExternalIPAddress"] = "192.168.255.169";
  values[VSOL_TR069 + ".X_CT-COM_VLANIDMark"] = 100;
  values[WCD2 + ".X_CT-COM_WANGponLinkConfig.VLANIDMark"] = 100;
  return values;
}
...
describe("gf-tr069-vlan1000", () => {
  ...
  it("finds TR-069 by VLAN 100 DHCP IP even when it is not WCD.2", () => {
    const values = vsolInverted();
    values[VSOL_INET + ".ExternalIPAddress"] = "10.64.0.40";
    values[VSOL_INET + ".X_CT-COM_VLANIDMark"] = 1;
    values[WCD1 + ".X_CT-COM_WANGponLinkConfig.VLANIDMark"] = 1;
    values[WCD1 + ".WANIPConnection.1.ExternalIPAddress"] = "192.168.253.40";
    values[WCD1 + ".WANIPConnection.1.X_CT-COM_VLANIDMark"] = 100;
    values[WCD1 + ".X_CT-COM_WANGponLinkConfig.VLANIDMark"] = 100;
    ...
    values[WCD2 + ".WANIPConnection.1.ExternalIPAddress"] = "10.64.0.40";
    ...
    assert.equal(sets[WCD1 + ".WANIPConnection.1.X_CT-COM_VLANIDMark"], 1000);
    assert.equal(sets[WCD1 + ".X_CT-COM_WANGponLinkConfig.VLANIDMark"], 1000);
    assert.equal(sets[VSOL_INET + ".X_CT-COM_VLANIDMark"], undefined);
  ...
  it("does not probe missing WANPPP 1..8 on a dual-IP VSOL", () => {
    const ops = runProvision("V2804AX15T", customerDualIpWan());
    const sets = setsOf(ops);
    assert.equal(sets[VSOL_TR069 + ".X_CT-COM_VLANIDMark"], 1000);
    assert.equal(sets[WCD1 + ".WANIPConnection.1.X_CT-COM_VLANIDMark"], undefined);
    const burned = ops.declared.filter((p) =>
      /WAN(?:PPP|IP)Connection\.[3-8]$/.test(p) || /WANConnectionDevice\.[3-8]$/.test(p),
    );
    assert.deepEqual(burned, []);
    const ppp = ops.declared.filter((p) => p.indexOf("WANPPPConnection") !== -1);
    assert.deepEqual(ppp, []);
  });

  it("discovers with cached path timestamps not Date.now", () => {
    assert.equal(/declare\([^)]*Date\.now\(\)/.test(SCRIPT), false);
    assert.equal(SCRIPT.includes("{ path: 1 }"), true);
  });
});
