...
// Discovers WANs by probing WCD/WAN instance .path (GenieACS declare is not an array).
...
function probeExists(path) {
  const declared = declare(path, { path: Date.now() });
  return !!(declared && declared.path);
}

function collectConnections(layout, wans, wcdPath, connType) {
  for (let i = 1; i <= 8; i++) {
    const wanPath = wcdPath + "." + connType + "." + i;
    if (!probeExists(wanPath)) continue;
...
function collectWans(layout) {
  const wans = [];
  for (let wcdId = 1; wcdId <= 8; wcdId++) {
    const wcdPath = WAN_DEVICE + ".WANConnectionDevice." + wcdId;
    if (!probeExists(wcdPath)) continue;
    collectConnections(layout, wans, wcdPath, "WANIPConnection");
    collectConnections(layout, wans, wcdPath, "WANPPPConnection");
  }
  return wans;
}
...
if (!layout) {
  ...
  const wans = collectWans(layout);
  logStep("wans=" + wans.length + " cr=" + cr);
  const wan = findTr069Wan(wans, cr);
  if (!wan) {
