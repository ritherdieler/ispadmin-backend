# MK1: GRE tunnel con VPS ispAdmin + enrutamiento a OLT (sin DST-NAT)
# VPS 212.85.13.47 <-> MK1 38.224.231.2
# Tunnel: 10.255.255.0/30
# SSH destino real: 10.11.104.2:22

/interface gre remove [find name=gre-ispadmin-vps]
/interface gre add name=gre-ispadmin-vps local-address=38.224.231.2 remote-address=212.85.13.47 \
    allow-fast-path=no comment="ispAdmin VPS GRE -> OLT LAN"

/ip address remove [find interface=gre-ispadmin-vps]
/ip address add address=10.255.255.1/30 interface=gre-ispadmin-vps comment="ispAdmin VPS GRE"
...
/ip firewall nat remove [find comment="IspAdmin VPS -> OLT SSH"]
/ip firewall nat remove [find comment="IspAdmin VPS -> OLT SSH SNAT"]
