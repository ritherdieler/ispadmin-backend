...
class RouterOsClientConfigTest {
    ...
    @Test
    fun `catalog does not declare mikrotik-java legrange dependency`() {
        val catalog = Path.of(System.getProperty("user.dir")).resolve("gradle/libs.versions.toml").readText()
        assertThat(catalog).doesNotContain("GideonLeGrange")
        assertThat(catalog).doesNotContain("mikrotik-java")
    }
}
