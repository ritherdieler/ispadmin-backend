...
| ACS WAR (TR-069) | `ACS_API_KEY` (secreto; header `X-Acs-Key`; misma pareja Gateway ↔ ACS WAR → `acs.api-key` / `olt.gateway.acs.api-key`). `ACS_PROFILES_CATALOG` → `acs.profiles.catalog` (schema MySQL con `tr069_model_profile`; **no** poner en `.env` compartido: horneado en el WAR — staging `ispadmin_staging`, prod `ispadmin`). `OLT_GATEWAY_ACS_BASE_URL` **no** va en `.env` compartido: horneada en el WAR gateway (`olt.gateway.acs.internal-base-url`). Staging: `http://127.0.0.1:8080/ispadmin-staging-acs`. JDBC `stg_acs` / `prod_acs`. Core no habla con ACS WAR ni GenieACS. |
