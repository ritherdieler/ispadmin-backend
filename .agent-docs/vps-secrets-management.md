...
| Base de datos backend | `DB_PASSWORD` → `spring.datasource.password` en `application-prod.properties`. Debe coincidir con `MYSQL_ROOT_PASSWORD` de `mysql8033` y con `SPRING_DATASOURCE_PASSWORD` del servicio `tomcat` en `docker-compose.yml`, que hoy tiene precedencia sobre el WAR (ver «Secretos en Docker Compose»). |
| Telemetría (esquema aparte) | `TELEMETRY_DATASOURCE_URL`, `TELEMETRY_DATASOURCE_USERNAME`, `TELEMETRY_DATASOURCE_PASSWORD` → `telemetry.datasource.*`. Vacío = mismo MySQL que el negocio. Corte: apuntar a `jdbc:mysql://mysql:3306/ispadmin_telemetry?...` (esquema creado por Flyway `V43`). |
| Escrituras ONU (F7) | `OLT_PROVIDER_AUTHORIZE`, `OLT_PROVIDER_DELETE`, `OLT_PROVIDER_REBOOT`, `OLT_PROVIDER_MOVE` (`SMARTOLT` o `GATEWAY`; default `GATEWAY`); `OLT_PROVIDER_AUTHORIZE_SHADOW` (default `true`, útil si se fuerza SMARTOLT) → `olt.provider.*`. |
| SmartOLT (SaaS en retirada) | `OLT_SERVICE_API_KEY` → `olt.service.api-key`; `OLT_SERVICE_BASE_URL` → `olt.service.base-url` (default `https://gigafiberperu.smartolt.com/api/`); `OLT_SERVICE_CONNECT_TIMEOUT_MS` (default `5000`) y `OLT_SERVICE_READ_TIMEOUT_MS` (default `15000`) → timeouts de `SmartOltHttpClient`. La key salió del binario (`ConfigConstants.kt` eliminado) y **debe rotarse** en el panel de SmartOLT. Con `olt.service.mock.enabled=true` (dev) la key no se usa. |
...
Rotación MySQL: actualizar **compose (`MYSQL_ROOT_PASSWORD` y `SPRING_DATASOURCE_PASSWORD`) + `DB_PASSWORD` en `.env`** y reiniciar servicios. `application-prod.properties` ya no lleva valor.
...
- SmartOLT API key y password MySQL **ya no están en el JAR**: `application-prod.properties` usa `${OLT_SERVICE_API_KEY:}` y `${DB_PASSWORD}`. Pagos Micuentaweb sigue pendiente de migrar. El contenedor **no** debe fijar `SPRING_DATASOURCE_URL`.
- **Antes del próximo deploy** conviene declarar `DB_PASSWORD` y `OLT_SERVICE_API_KEY` en `/opt/gigafiber/.env`. La base sigue arrancando sin `DB_PASSWORD` porque `SPRING_DATASOURCE_PASSWORD` del compose gana al WAR, pero SmartOLT deja de responder (401) si falta `OLT_SERVICE_API_KEY`.
