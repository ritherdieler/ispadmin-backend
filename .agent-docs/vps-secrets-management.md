...
Host de referencia: KVM4 `2.24.66.53` (`srv1992868`), stack en `/opt/gigafiber/`. No desplegar en `212.85.13.47`.
...
| RouterOS REST | `ROUTER_OS_REST_TRUSTSTORE_PASSWORD` → truststore JKS embebido. `ROUTER_OS_CLIENT_ADAPTER` y `router.os.client.classic.*` siguen existiendo como leftovers **deprecados** (2026-09-16); el único transporte es `/rest` :443. |
...
| OLT Gateway (SSH) | No hay `OLT_GATEWAY_ENABLED`: el módulo no se apaga. `OLT_GATEWAY_CLIENT_ENABLED` (default `true`). `OLT_GATEWAY_API_KEY` (Core prod → `X-Olt-Gateway-Key`, escrituras de flota). `OLT_GATEWAY_STAGING_API_KEY` (secreto distinto; Core staging/prestaging → misma header; el Gateway la acepta en `olt.gateway.staging-api-key` y limita escrituras a SN lab). `OLT_GATEWAY_PASSWORD`, `OLT_GATEWAY_HOST`, `OLT_GATEWAY_USERNAME` (VPS actual `212.85.13.47` sigue en `oltadmin`. `gfbackend` es solo el VPS nuevo KVM4 `2.24.66.53`; ver `olt-usuario-gfbackend.md`), `OLT_GATEWAY_WRITES_ENABLED` (prod: env, default `false`; el dueño SSH lo necesita `true` para el alta lab). `OLT_GATEWAY_SSH_MAX_RETRY_ATTEMPTS` (default 0=unlimited), `OLT_GATEWAY_SSH_RETRY_DELAY_MS` (default 2000) → `olt.gateway.ssh.*`. `OLT_GATEWAY_V2_WRITE_MAX_RETRY_ATTEMPTS` (default 2) → `olt.gateway.writes.v2-write-max-retry-attempts`; limita específicamente una etapa v2 y deja el error reintentable en el journal, incluso si el comportamiento legado es ilimitado. |
| OLT Gateway WAR (split) | `OLT_GATEWAY_INTERNAL_BASE_URL` y `OLT_GATEWAY_DATASOURCE_URL` **no** van en `.env` compartido: van horneadas (`olt.gateway.internal-base-url`, JDBC `stg_oltgateway` / `prod_oltgateway`). Staging core (stopgap) → `http://tomcat9027:8080/ispadmin` (horneado; no es loopback al propio staging). Prestaging Mac → `${OLT_GATEWAY_INTERNAL_BASE_URL:http://127.0.0.1:8092/ispadmin}`. `olt_mgr_*` vive en el schema del módulo gateway; `olt_mgr_onu_optical_sample` permanece en el schema del core. `OLT_GATEWAY_OPERATION_SECRET` → `olt.gateway.operation-secret` (AES-GCM del journal de activación; VPS `/opt/gigafiber/.env`; fallback `${OLT_GATEWAY_API_KEY}` si falta). Rotar implica re-cifrar u operaciones en curso ilegibles. |
| SmartOLT cloud (toggle de escrituras en Gateway) | `OLT_SERVICE_BASE_URL` → `olt.service.base-url` (default `https://gigafiberperu.smartolt.com/api/`); `OLT_SERVICE_API_KEY` (secreto; header `X-Token`) → `olt.service.api-key`; `OLT_SERVICE_CONNECT_TIMEOUT_MS` / `OLT_SERVICE_READ_TIMEOUT_MS`. Toggle por operación: `OLT_PROVIDER_AUTHORIZE`, `OLT_PROVIDER_DELETE`, `OLT_PROVIDER_REBOOT`, `OLT_PROVIDER_MOVE` (`SMARTOLT` \| `GATEWAY`, default `GATEWAY`) → `olt.provider.*`. Homologación SSH↔SmartOLT: `OLT_GATEWAY_CUSTOM_PROFILE_BINDINGS` → `olt.gateway.writes.custom-profile-bindings` (default `Generic_1:1=3:2,Generic_1:100=6:13`); `OLT_GATEWAY_INBOUND_TRAFFIC_TABLE` / `OLT_GATEWAY_OUTBOUND_TRAFFIC_TABLE` → traffic-table 8/9; `OLT_GATEWAY_SMARTOLT_OLT_ID` → `olt.gateway.smartolt-olt-id` (default `2`). ONU lab ACS (sufijos `0031C0B6`, `12345B4641531C0B6`, SN `ZTEGDC47BFFD`): `OLT_GATEWAY_LAB_ACS_SN_SUFFIXES`, `OLT_GATEWAY_LAB_ACS_LINE_PROFILE_ID` (12), `OLT_GATEWAY_LAB_ACS_MGMT_VLAN` (1000), `OLT_GATEWAY_LAB_ACS_MGMT_GEMPORT` (2) — no son secretos; defaults horneados. El WAR Gateway las hornea en `application-oltgateway.properties` (el overlay no hereda `application-dev/prod`). Core las usa solo si `olt.gateway.client-enabled=false`. Viven en `/opt/gigafiber/.env`. |
| SmartOLT authorize mgmt VLAN | `OLT_SMARTOLT_CUSTOMER_VLANS_WITH_MGMT` (default `100`), `OLT_SMARTOLT_MGMT_VLAN` (default `1000`) → `olt.smartolt.*`; post-authorize `onu/set_onu_mgmt_ip_dhcp/{sn}` sin SSH cuando Core autoriza por cloud (`olt.gateway.client-enabled=false`). |
| ACS WAR (TR-069) | `ACS_API_KEY` (secreto; header `X-Acs-Key`; misma pareja **Core ↔ ACS WAR** y Gateway ↔ ACS WAR → `acs.api-key` / `olt.gateway.acs.api-key`). **Prod overlay** (`application-prod.properties`, perfil solo `prod`) debe hornear `olt.gateway.acs.api-key=${ACS_API_KEY:}`: el WAR único **no** carga `application-oltgateway.properties`. Sin esa línea el Gateway manda `X-Acs-Key` vacío y ACS responde 401 (alta #2363 2026-09-17). Core: `acs.client-enabled` (`ACS_CLIENT_ENABLED`, staging horneado `true`) y `acs.internal-base-url` (`ACS_INTERNAL_BASE_URL`; **no** en `.env` compartido; staging Core `http://127.0.0.1:8080/ispadmin-staging`). `GET /api/acs/v1/cpe/{sn}/access-layout` y `POST /api/acs/v1/cpe/provision` los llama el Core (`wispadmin/acsclient`) y el Gateway. `GENIEACS_TO_ACS_API_KEY` (secreto; GenieACS `ext` → ACS `POST /api/acs/v1/cpe/inform-notify`; también acepta `ACS_API_KEY` en ese endpoint → `acs.genieacs-to-acs-api-key`). `ACS_TO_GATEWAY_API_KEY` (secreto; el gateway lo sigue aceptando en `olt.gateway.acs-to-gateway-api-key`). El Inform del ACS no usa esa key como llamante: `acs.gateway.api-key` es `OLT_GATEWAY_API_KEY` en prod y `OLT_GATEWAY_STAGING_API_KEY` en staging/prestaging, header `X-Acs-To-Gateway-Key`, más `X-Gigafiber-Env` horneado en `acs.gateway.env` (`prod`, `stg`, `lpstg`). Con llamante staging, un serial fuera de `olt_lab_onu` responde 403 y no entra al stream. `ACS_GATEWAY_BASE_URL` → `acs.gateway.internal-base-url` en prod (loopback `/ispadmin`). Staging hornea `http://tomcat9027:8080/ispadmin`. Prestaging hornea `http://127.0.0.1:8082/ispadmin` y el header `lpstg` escribe en el stream `lpstg`. `GENIEACS_TO_ACS_NOTIFY_URL` vive en `/opt/gigafiber/genieacs/.env` (base URL **prod** del ACS WAR vista desde GenieACS, p. ej. `http://tomcat9027:8080/ispadmin`). `GENIEACS_TO_ACS_STAGING_NOTIFY_URL` (mismo archivo; p. ej. `http://tomcat-staging:8080/ispadmin-staging`) solo la usan las ONU lab (`ZTEGDC47BFFD`, `12345B4641531C0B6`): el ext `wifi-inform-notify` hace POST a prod **y** staging. El resto de la flota solo pega a `GENIEACS_TO_ACS_NOTIFY_URL`. Perfiles TR-069 (`tr069_model_profile`) viven en JDBC `stg_acs` / `prod_acs` (Flyway ACS `db/acs`). No hay `ACS_PROFILES_CATALOG`. `OLT_GATEWAY_ACS_BASE_URL` sigue siendo el fallback de un solo llamante (`olt.gateway.acs.internal-base-url`). Con dos Cores: `OLT_GATEWAY_ACS_BASE_URL_PROD` → `olt.gateway.acs.base-url-prod` y `OLT_GATEWAY_ACS_BASE_URL_STAGING` → `olt.gateway.acs.base-url-staging`. El overlay staging trae el default Docker `http://tomcat-staging:8080/ispadmin-staging`; `/opt/gigafiber/.env` puede declararlo explícitamente para que el routing no dependa del orden de perfiles. El overlay prod puro mantiene el valor vacío y exige esa variable cuando atiende llamadas `stg`. `olt.gateway.acs.require-caller=true` en prod: sin `X-Gigafiber-Env` el day-2 responde 400. Staging in-process conserva `http://127.0.0.1:8080/ispadmin-staging` como URL local del ACS, pero el router por llamante usa la URL Docker anterior. Clientes externos (Android, backoffice) siguen hablando **solo con el Core**. |
| OLT Gateway SNMP RO | `OLT_GATEWAY_SNMP_ENABLED`, `OLT_GATEWAY_SNMP_RO_COMMUNITY` (secreto), `OLT_GATEWAY_SNMP_PORT`, `OLT_GATEWAY_SNMP_TIMEOUT_MS` (default WAR **20000**), `OLT_GATEWAY_SNMP_RETRIES` (default **2**; el agente descarta ~2-8% de páginas GETBULK sin PDU de error), `OLT_GATEWAY_SNMP_MAX_REPETITIONS` (default 25; óptimo medido, no bajar en prod sin job A/B), `OLT_GATEWAY_SNMP_REQUEST_INTERVAL_MS` (default 100; pacing entre páginas), `OLT_GATEWAY_SNMP_OPTICAL_PARALLEL_COLUMNS` (default **false**; **no-op** desde el GETBULK multi-varbind, se mantiene solo para rollback de overrides), `OLT_GATEWAY_SNMP_OPTICAL_PER_PORT` (default **true**; alineado staging=prod), `OLT_GATEWAY_SNMP_OPTICAL_PARALLEL_PORTS` (default **1**; la ruta OMCI del MA5608T es serial, subirlo empeora el wall-clock), `OLT_GATEWAY_SNMP_OPTICAL_ONLINE_ONLY` (default true; solo si PER_PORT), `OLT_GATEWAY_SNMP_FUSED_INVENTORY_OPTICAL` (default **true**; una sola pasada per-port serial de 13 columnas que trae inventario y óptica juntos, −51% de wall-clock; **inmune** a `PER_PORT`/`PARALLEL_PORTS`: si fused=true fuerza serial y loguea WARN si esas flags contradicen; no falla el arranque), `OLT_GATEWAY_SNMP_FUSED_SNAPSHOT_MAX_AGE_MS` (default 900000 = 15 min; edad máxima del snapshot de inventario que la pasada fusionada deja en caché para el sync), `OLT_GATEWAY_SNMP_INVENTORY_TIMEOUT_MS` (default **5000**; solo jobs INVENTORY/AUTOFIND, cuyas páginas son de 100-800 ms; la óptica sigue con TIMEOUT_MS=20000), `OLT_GATEWAY_SNMP_INVENTORY_RETRIES` (default **4**; pareja del anterior), `OLT_GATEWAY_SNMP_ACQUIRE_TIMEOUT_MS` (default 300000; espera de permit del bus de esa OLT), `OLT_GATEWAY_SNMP_POLL_LOCK_ENABLED` (default true; lock del poll óptico), `OLT_GATEWAY_SNMP_POLL_LOCK_KEY` (default `olt-snmp-poll`; **sin** prefix `stg:`/`prod:` si `POLL_LOCK_SHARED=true`), `OLT_GATEWAY_SNMP_POLL_LOCK_TTL_MS` (default 1200000 ≈ 20 min), `OLT_GATEWAY_SNMP_POLL_LOCK_SHARED` (default true; prod y staging ceden el uno al otro), `OLT_GATEWAY_SNMP_TRAP_ENABLED` (default false; receptor ASN.1 Huawei), `OLT_GATEWAY_SNMP_TRAP_LISTEN_PORT` (default 1162; ≠ NetDiag 1620), `OLT_GATEWAY_SNMP_TRAP_BIND_ADDRESS`, `OLT_GATEWAY_SNMP_TRAP_COMMUNITY` (opcional; secreto si se usa), `OLT_GATEWAY_SNMP_TRAP_BUFFER_SIZE`, `OLT_GATEWAY_SNMP_TRAP_DISPATCHER_THREADS`, `OLT_GATEWAY_SYNC_SIGNAL_INTERVAL_MS` (default 300000 = 5 min; también slice de espera del lock), `OLT_GATEWAY_SYNC_LAB_OPTICAL_SSH_ENABLED` (default `false`; el SSH lab vive en el WAR gateway, no en el overlay del core), `OLT_GATEWAY_SYNC_LAB_OPTICAL_SSH_INTERVAL_MS` (default `900000`), `OLT_GATEWAY_SYNC_LAB_OPTICAL_SSH_INITIAL_DELAY_MS` (default `120000`) → `olt.gateway.snmp.*` / `olt.gateway.sync.*`. Capacidad SNMP GET: `OltSnmpModelLimits` / `max_concurrent_snmp_walks` (MA5608T=1). **No usar** `OLT_GATEWAY_SNMP_ALLOW_SSH_FALLBACK` ni `OLT_GATEWAY_SNMP_ALLOW_SSH_SIGNAL_FALLBACK` (eliminadas 2026-09-16; inventario y óptica van solo por SNMP). Detalle: `olt-snmp-optical-contention.md`, `remove-ssh-fallbacks-2026-09-16.md`. |
...
| Diagnóstico 360 / service-health | `SERVICE_HEALTH_ENABLED`, `SERVICE_HEALTH_OPTICAL_ENABLED`, `SERVICE_HEALTH_ACS_ENABLED`, `SERVICE_HEALTH_CORRELATION_ENABLED`, `SERVICE_HEALTH_SHARED_INCIDENTS_ENABLED`, `SERVICE_HEALTH_ACTIONS_ENABLED`, `SERVICE_HEALTH_CONFIG_ENABLED`, `SERVICE_HEALTH_STATION_HMAC_KEY` (secreto ≥32 bytes; exigido al activar ACS/acciones; no rotar sin plan de `station_key`), `SERVICE_HEALTH_ACS_GPV_COOLDOWN_SECONDS` (default prod `1800`; cooldown entre GPV WLAN automáticos del watcher), `SERVICE_HEALTH_ACS_WIFI_SAMPLE_TARGET_SECONDS` (default prod `1800`; cadencia objetivo de muestras Wi‑Fi), `SERVICE_HEALTH_STATION_HOURLY_RETENTION_DAYS` (default `90`), `SERVICE_HEALTH_STATION_SERIES_RAW_MAX_DAYS` (default `7`; series ≤7 d en raw, más largas en hourly), `SERVICE_HEALTH_OPTICAL_RETENTION_DAYS` (default `90`; raw óptico), `SERVICE_HEALTH_OPTICAL_DAILY_RETENTION_DAYS` (default `730`; roll-up diario), `SERVICE_HEALTH_OPTICAL_SERIES_RAW_MAX_DAYS` (default `90`; series ≤90 d en raw, más largas en daily), `SERVICE_HEALTH_SNAPSHOT_FRESH_SECONDS` (default `60`; frescura de `service_health_current` en el GET 360) → `service.health.*`. Default todo `false`. **Lab:** no hay env; membresía = `subscription_acs.lab` (tag GenieACS `lab` vía `SubscriptionAcsSyncService`) para candado de writes e Inform 60 s. No hay lista piloto ni exclusión lab como criterio de recolección. **No usar** `SERVICE_HEALTH_LAB_SUBSCRIPTION_IDS`, `SERVICE_HEALTH_PILOT_SUBSCRIPTION_IDS`, `SERVICE_HEALTH_PILOT_ACS_DEVICE_IDS`, `SERVICE_HEALTH_SHARED_INCIDENT_NOTIFICATIONS_ENABLED`, `SERVICE_HEALTH_OPTICAL_PULL_ENABLED` ni `SERVICE_HEALTH_ACS_POLL_ENABLED` (eliminadas; no hay binding). Quitadas de `/opt/gigafiber/.env` el 2026-09-16. Detalle: `remove-collection-gate-2026-09-15.md`, `remove-optical-http-pull-2026-09-16.md`. **Staging:** `application-staging.properties` fija `service.health.acs-wifi-sample-target-seconds=180` y `acs-gpv-cooldown-seconds=180` (3 min; no poner esos `SERVICE_HEALTH_ACS_*_SECONDS` en `/opt/gigafiber/.env` o ganarían al WAR). Detalle: `staging-acs-wifi-sample-cadence-3min.md`, `service-health-lab-membership-db-only-2026-09-07.md`, `optical-daily-rollup-retention.md`. As-built: `01-implementacion-analitica-consumo-ancho-banda.md` (tráfico) y `03-implementacion-diagnostico-tecnico-convergente.md`. |
| Redis interno (cache 360 + Streams) | `REDIS_ENABLED` (staging core: default `true` vía `application-staging`; prod default `false` vía `${REDIS_ENABLED:false}`; **VPS prod desde 2026-09-17:** `REDIS_ENABLED=true` en `/opt/gigafiber/.env` + recreate `tomcat9027`, ver `prod-redis-360-2026-09-17.md`; **WAR oltgateway** fuerza `gigafiber.redis.enabled=true` en bake para publicar `cpe.provisioning`). `REDIS_HOST` (VPS: hostname compose `redis`; no exponer a internet), `REDIS_PORT` (default `6379`), `REDIS_PASSWORD` (secreto; `openssl rand -hex 16`). Enlazan `gigafiber.redis.*`. Compose VPS: servicio `redis` en `/opt/gigafiber/docker-compose.yml` (`maxmemory 128mb`, AOF). Local: `docker-compose.redis.yml` + `scripts/redis-local.sh`. Stream `gigafiber.events`, grupos `snapshot-core` (360) y `cpe-provision-core` (cierre `tr069` en core). No poner la password en git. |
...
| Tráfico WAR (collector) | `TRAFFIC_API_KEY` (secreto; header `X-Traffic-Key`; misma key core↔traffic). `TRAFFIC_CLIENT_ENABLED` → `traffic.client-enabled` (fachada Core `/subscription/{id}/traffic/*`; staging/prestaging hornean `true`; prod default `false` en `application.properties`; **VPS prod desde 2026-09-17:** `true` en `/opt/gigafiber/.env`, ver `prod-redis-360-2026-09-17.md`). `TRAFFIC_INTERNAL_BASE_URL` y `TRAFFIC_CORE_BASE_URL` **no** van en `.env` compartido: van horneadas en el WAR (`traffic.internal-base-url` / `traffic.core-base-url`). Staging: core → `http://127.0.0.1:8080/ispadmin-staging`; traffic → `http://127.0.0.1:8080/ispadmin-staging`. JDBC schema propio `stg_traffic` / `prod_traffic` (migrate SQL, no ejecutar sin OK). |
...
openssl rand -hex 16   # OLT_GATEWAY_STAGING_API_KEY   # distinta de OLT_GATEWAY_API_KEY
```
...
### Alta v2 — entrega durable de telemetría (2026-09-22)

Plantilla: `scripts/provisioning-v2.env.example`. En KVM4 staging, los valores de v2 viven en `/opt/gigafiber/.env.staging-v2` (permisos `0600`) y solo los carga `tomcat-staging`; no añadirlos al `.env` compartido con producción.

| Variable | Entorno / ubicación | Propiedad / pareja |
|---|---|---|
| `PROVISIONING_V2_ENABLED` | Core local/staging/prod; entorno del proceso | `provisioning.v2.enabled`, false por defecto. Solo habilita solicitudes explícitas con `provisioningFlowVersion=2`; no cambia el flujo v1. En KVM4 se habilitó solo en staging para homologación. |
| `PROVISIONING_V2_WORKER_ENABLED` | Solo Core; entorno del proceso | `provisioning.v2.worker-enabled`, false por defecto. Registra executor y recuperación de v2; en KVM4 se habilitó solo después de migraciones y publicación ACS en staging. |
| `PROVISIONING_V2_TR069_PROFILE_ID` | Core local/staging/prod; entorno del proceso | `provisioning.v2.tr069-profile-id`, `0` por defecto para impedir una selección accidental. Antes de habilitar v2 fijar el ID real del perfil TR-069 validado para ese entorno; queda cifrado en el snapshot de la operación para que un reintento no tome un valor posterior. |
| `PROVISIONING_V2_ACS_BASELINE_KEY` | Solo proceso ACS y almacén de secretos del entorno; KVM4 staging: `/opt/gigafiber/.env.staging-v2` | `acs.provisioning-v2-baseline-key`. Clave distinta y no vacía para AES-GCM de snapshots Wi‑Fi v2 en schema ACS. Nunca viaja a Core, Gateway, Android ni backoffice; si falta, el paso Wi‑Fi falla antes de mutar la ONU. |
| `GENIEACS_V2_PUBLISH_ENABLED` | Solo proceso ACS, activación temporal de despliegue | `genieacs.v2-publish-enabled`, false por defecto. Publica una vez los IDs v2 empaquetados y registra SHA-256; volver a false tras confirmar el despliegue. |
| `PROVISIONING_V2_TELEMETRY_ENABLED` | Core local/staging/prod; entorno del proceso | `provisioning.v2.telemetry-enabled`, false por defecto. Requiere V56 en emisor y V57 en receptor. |
| `PROVISIONING_V2_OBS_BASE_URL` | Core; properties locales o `/opt/gigafiber/.env` al desplegar | `provisioning.v2.observability-base-url`; URL del Core receptor incluido contexto. |
| `PROVISIONING_V2_OBS_API_KEY` | Emisor y receptor; almacén de secretos local o VPS | `provisioning.v2.observability-api-key` ↔ `observability.api-keys.backend`. Rotar ambos juntos; nunca enviar al cliente. |
| `PROVISIONING_V2_OUTBOX_DELAY_MS` | Core; entorno del proceso | `provisioning.v2.outbox-delay-ms`, 5000 por defecto. |

Entrega HTTP: `X-Obs-Delivery-Id` identifica un evento inmutable, no es secreto. El receptor confirma ese mismo encabezado solo tras persistencia. Una respuesta parcial no vacía el outbox.
