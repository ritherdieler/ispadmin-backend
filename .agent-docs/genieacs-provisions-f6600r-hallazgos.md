...
| `wildcardSize` undefined → `nextCount(..., 0)` = 1 | Alta ZTE #17 2026-09-16: `gf-pppoe-wan2-poc add WANPPPConnection.* count=undefined -> 1`. Creó `.1` vacía, escribió hojas en `.2` (no existe). GPV sin `10.64.*` → TR-069 PENDING. | Slot internet F6600 = `WANPPPConnection.2`. Si `size` no es un entero > 0, `desired = slot` (2), no 1. Borrar leftover IP/PPP extra. TR-069 (`WANIP.1`) no se toca. |
