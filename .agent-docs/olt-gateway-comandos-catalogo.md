...
| `POST /onus/{sn}/service-port/ensure-mgmt` body `{"vlan":1000}` | Pre-registra SP de gestión: lineprofile 12 + VLAN 1000 gem 2 | `OltServicePortService.ensureMgmtVlan` / Core `POST /onu/{sn}/service-port/ensure-mgmt` / `retag-tr069-vlan1000.sh` | Idempotente. Skip si el display ya lista la VLAN. Core es la fachada; el runner no llama al Gateway ni abre SSH. **No usar** en ONU con internet VLAN 1: el `ont modify … 12` quita el GEM de VLAN 1. Restore: [restore-vsol-vlan1-keep1000-2026-09-18.md](./restore-vsol-vlan1-keep1000-2026-09-18.md) |
...
| `ont-lineprofile gpon profile-id {id} profile-name "{name}"` | Entra/crea el profile (vista `config-gpon-lineprofile-{id}`) | Alta VLAN 1000 | Crear uno nuevo en vez de editar uno con bindings: el `commit` reconfigura por OMCI a **todas** las ONUs vinculadas |
...
| `gem mapping {gem} {index} vlan {vlan}` | Mapea una VLAN de usuario a un GEM | Segunda VLAN en la misma ONU | La segunda VLAN necesita **su propio gem** (`gem 2`), no basta con el service-port |
| `commit` | Persiste el profile | Cierre de edición | Obligatorio antes de `quit` |
| `ont modify {port} {ontId} ont-lineprofile-id {id}` | Revincula la ONT a otro line profile (dentro de `interface gpon 0/{slot}`) | `OltGatewayCommandService.ensureMgmtServicePort` / `scripts/olt-restore-vsol-vlan1-keep1000.expect` | Falla `The GEM port cannot be deleted…` si un SP sigue en un GEM que el destino no tiene: `undo service-port port …` **antes**. Live 2026-09-18 `VSOL00872649` `0/1/1` ont 91: 12→**13** (`Generic_1_HFD44D20E`, gem1 VLAN 100+1+1000) |
| `service-port vlan {vlan} gpon 0/{slot}/{port} ont {ontId} gemport 2 multi-service user-vlan {vlan} tag-transform translate inbound traffic-table index 8 outbound traffic-table index 9` | Segundo service-port de gestión en la misma ONU | `OltGatewayCommandService.ensureMgmtServicePort` / alta VLAN 1000 | `gemport` debe ser el que el line profile mapea a esa VLAN. Convive con el SP de internet: `display service-port port 0/{slot}/{port} ont {ontId}` devuelve 2 filas. Idempotente si el CLI responde `already exists` y el display ya lista la VLAN |
