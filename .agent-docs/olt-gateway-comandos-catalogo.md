...
| `gem mapping {gem} {index} vlan {vlan}` | Mapea una VLAN de usuario a un GEM | Segunda VLAN en la misma ONU | Puede ir en **gem 1** si el profile ya multiplexa VLANs (p. ej. `HD446E747` 100+1000). `Generic_1_V100` (id 6) tiene gem 2 vacío: `gem mapping 2 1 vlan 1000`. Un `commit` empuja OMCI a todos los bindings |
| `commit` | Persiste el profile | Cierre de edición | Obligatorio antes de `quit` |
| `ont modify {port} {ontId} ont-lineprofile-id {id}` | Revincula la ONT a otro line profile (dentro de `interface gpon 0/{slot}`) | Alta VLAN 1000 en ONU lab | Funciona con la ONT **offline**; `display ont info {port} {ontId}` confirma el nuevo `Line profile ID` |
| `service-port vlan {vlan} gpon 0/{slot}/{port} ont {ontId} gemport 2 multi-service user-vlan {vlan} tag-transform translate inbound traffic-table index 8 outbound traffic-table index 9` | Segundo service-port de gestión en la misma ONU | Alta VLAN 1000 | `gemport` debe ser el que el line profile mapea a esa VLAN. Convive con el SP de internet: `display service-port port 0/{slot}/{port} ont {ontId}` devuelve 2 filas |
...
| `undo service-port vlan {vlan} gpon 0/{slot}/{port} ont {ontId}` | Elimina el service-port de una VLAN | `OltGatewayCommandService.removeServicePort` | Cuarentena PPPoE retira VLAN 1; writes.enabled requerido |
