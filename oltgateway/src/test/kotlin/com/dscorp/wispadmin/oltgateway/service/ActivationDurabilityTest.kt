...
class ActivationDurabilityTest {
    @Test fun `restart preserves identity and resumes ACS without repeating authorization`() {
        ...
        val service=OnuActivationService(facade,acs,events,journal(), acsExecutor = Executor { })
        ...
        val restarted=OnuActivationService(facade,acs,events,journal(), acsExecutor = Executor { it.run() })
        restarted.recover()
