...
class HuaweiCliSessionKeepaliveTest {
    ...
    @Test
    fun `prepareSession deja sesion en config con mmi-mode y scroll`() {
        val written = setupShellMocks(captureWrites = true)
        ...
        val configIdx = joined.indexOf("config")
        val mmiIdx = joined.indexOf("mmi-mode enable")
        val scrollIdx = joined.indexOf("scroll 512")
        assertTrue(configIdx >= 0 && mmiIdx > configIdx && scrollIdx > mmiIdx)
        val afterScroll = joined.substring(scrollIdx)
        assertTrue(!afterScroll.contains("quit\r") && !afterScroll.contains("quit\n"))
    }
