...
class OnuActivationService(
    ...
    private val defaultCallerEnv: String = "",
) {
    ...
    fun activate(rawRequest: OnuActivateRequestDto): OnuActivateResponseDto {
        ...
        if(operation.stage=="ACS_STATUS") {
            val remote=withAcsCaller { runCatching { acsCpeClient.status(request.sn) }.getOrNull() }
    ...
    private fun dispatchAcs(operation: ActivationOperation) {
        val env = GatewayCallContext.env()?.trim().orEmpty().ifBlank { defaultCallerEnv.trim() }
        acsExecutor.execute {
            try {
                if (env.isNotEmpty()) GatewayCallContext.setEnv(env)
                runAcs(operation)
            } finally {
    ...
    private inline fun <T> withAcsCaller(block: () -> T): T {
        if (!GatewayCallContext.env().isNullOrBlank()) return block()
        val fallback = defaultCallerEnv.trim()
        if (fallback.isEmpty()) return block()
        GatewayCallContext.setEnv(fallback)
        return try {
            block()
        } finally {
            GatewayCallContext.clear()
    ...
    fun provisionCpe(sn: String, request: AcsCpeProvisionRequest): AcsCpeProvisionResponse {
        val normalized = sn.trim().uppercase()
        return withAcsCaller { acsCpeClient.provision(request.copy(sn = normalized)) }
    }
    ...
    private fun runAcs(operation: ActivationOperation) {
        ...
        try {
            ...
            operation.leaseUntil=System.currentTimeMillis()+30_000
            journal.save(operation)
            log.warn("ACS outcome unconfirmed for {} operation {}: {}", request.sn, operation.operationId, ex.message)
        }
    }
