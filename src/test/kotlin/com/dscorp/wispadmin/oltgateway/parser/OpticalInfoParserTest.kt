...
class OpticalInfoParserTest {
    ...
    @Test
    fun `parsea formato detalle KV display ont optical-info port ontId`() {
        val output = FixtureLoader.load("display-ont-optical-info-detail-live.txt")
    ...
    @Test
    fun `parse prefiere fila de tabla cuando hay tabla y detalle`() {
        val table = FixtureLoader.load("display-ont-optical-info-all-live.txt")
        val detail = FixtureLoader.load("display-ont-optical-info-detail-live.txt")

        val result = parser.parse(table + "\n" + detail, ontId = 1)
