...
class OltSshClientTest {
    ...
    @Test
    fun `no habilita heartbeat de transporte MINA`() {
        val client = SshClient.setUpDefaultClient()
        OltSshClient.applyTransportTimeouts(client, properties)

        assertEquals(Duration.ZERO, CoreModuleProperties.HEARTBEAT_INTERVAL.get(client).orElse(null))
        assertEquals(Duration.ZERO, CoreModuleProperties.HEARTBEAT_REPLY_WAIT.get(client).orElse(null))
    }
