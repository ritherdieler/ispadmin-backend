...
class ParallelOnuInventoryReaderTest {
    ...
    private fun stubSessionExecute(handler: (String) -> String) {
        every { session.execute(any<String>()) } answers { handler(firstArg()) }
        every { session.execute(any<String>(), any<Long>()) } answers { handler(firstArg()) }
    }

    private fun stubInventoryBus() {
    ...
    @Test
    fun `listado masivo usa display ont info 0 slot all por cada slot gpon`() {
        ...
        stubSessionExecute { cmd ->
            commands += cmd
            when {
                cmd == "display board 0" -> board0
                cmd.startsWith("display board ") -> "% Parameter error\n"
                cmd == "display ont info 0 0 all" -> FixtureLoader.load("display-ont-info-0-all-live.txt")
                cmd == "display ont info 0 1 all" -> FixtureLoader.load("display-ont-info-0-slot-all-live.txt")
                else -> ""
            }
        }
        stubInventoryBus()
        ...
        assertEquals(1, commands.count { it == "display ont info 0 0 all" })
        assertEquals(1, commands.count { it == "display ont info 0 1 all" })
        assertTrue(commands.none { it == "display ont info 0 all" })
        assertTrue(commands.none { it.matches(Regex("""display ont info 0 \d+ \d+ all""")) })
        verify(exactly = 1) { session.execute("display ont info 0 0 all", any()) }
        verify(exactly = 1) { session.execute("display ont info 0 1 all", any()) }
        verify(exactly = 1) { cliBus.execute(CliJobType.INVENTORY, any<(HuaweiCliSession) -> Any>()) }
    }

    @Test
    fun `cuando slot-all falla hace fallback a port-all por puerto`() {
        val model = OltMgrOltModel(
            id = 1L,
            code = "MA5608T",
            maxConcurrentCliSessions = 4,
        ...
        every { oltRepository.findByName("gigafiber-ma5608t") } returns Optional.of(
            OltMgrOlt(id = 1L, name = "gigafiber-ma5608t", ipAddress = "10.11.104.2", model = model)
        )
        val portCommands = ConcurrentHashMap.newKeySet<String>()
        stubSessionExecute { cmd ->
            when {
                cmd == "display board 0" -> FixtureLoader.load("display-board-frame-0-chassis.txt")
                cmd.startsWith("display board ") -> "% Parameter error\n"
                cmd == "display ont info 0 1 all" -> throw OltCommandTimeoutException("CLI command timed out after 60000ms")
                cmd == "display ont info 0 0 all" -> FixtureLoader.load("display-ont-info-0-all-live.txt")
                cmd.matches(Regex("""display ont info 0 1 \d+ all""")) -> {
                    portCommands += cmd
                    FixtureLoader.load("display-ont-info-0-slot-all-live.txt")
                }
                else -> ""
            }
        }
        ...
        assertTrue(result.any { it.slot == 0 })
        assertTrue(result.any { it.slot == 1 })
        assertTrue(portCommands.isNotEmpty())
        assertTrue(portCommands.contains("display ont info 0 1 0 all"))
        assertTrue(portCommands.none { it == "display ont info 0 1 all" })
    }

    @Test
    fun `cuando todos los jobs fallan propaga la excepcion en lugar de lista vacia`() {
        val model = OltMgrOltModel(
            id = 1L,
            code = "MA5608T",
            maxConcurrentCliSessions = 4,
        ...
        stubSessionExecute { cmd ->
            when {
                cmd == "display board 0" -> FixtureLoader.load("display-board-frame-0-chassis.txt")
                cmd.startsWith("display board ") -> "% Parameter error\n"
                cmd.startsWith("display ont info 0 ") -> throw OltCommandTimeoutException("CLI command timed out after 180000ms")
                else -> ""
            }
        }
        stubInventoryBus()
        ...
        assertThrows(OltCommandTimeoutException::class.java) {
            reader.listOnusParsed()
        }
    }
    ...
    fun `dedupe por sn entre jobs sin fan-out de workers`() {
        ...
        stubSessionExecute { cmd ->
            when {
                cmd == "display board 0" -> FixtureLoader.load("display-board-frame-0-chassis.txt")
                cmd.startsWith("display board ") -> "% Parameter error\n"
                cmd.startsWith("display ont info 0 ") && cmd.endsWith(" all") -> {
